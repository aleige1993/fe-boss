<template>
  <div id="add-customer-owe-company">
    ...
    <i-table :height="tableFixHeight+98" :loading="loadingData" :columns="bankAccountColumns" :data="bankAccountDatas"></i-table>
    <!--添加联系人模态框-->
    ...
    <bs-modal title="选择企业" :width="1200" v-model="slectOweCompanyModal">
      <table-company-customer-list v-if="slectOweCompanyModal" ref="tableCompanyCustomer" type="modal" @on-row-dbclick="selectCompany">
  ...
  export default {
    ...
    methods: {
      selectCompany(row, index) {
        ...
        this.$data.formData.bizAddr = row.bizProvinceName + row.bizCityName + row.bizDistrictName + row.bizRoadAddr;
