<template>
  <div id="page-table-demo" class="page-carmodel-manage">
    ...
    <pt-modal :title="isAdd ? '添加车型' : '修改车型'" v-model="addModal" :width="1000" :zIndex="200" @on-close="fromData.modelFileUploading==false">
      <i-form v-if="addModal" ref="fromData" :model="fromData" label-position="right" :label-width="100">
        <i-row>
          <i-col span="12">
          ...
          <i-col span="12">
            <i-form-item label="子品牌名称" prop="childBrandName" :rules="{required: true, message: '子品牌名称不能为空'}">
              <div class="flex-items">
                <i-select :disabled="!fromData.brandNo" v-model="fromData.childBrandName" placeholder="">
                  <i-option v-for="item in childBrandList" :key="item.name" :value="item.name">{{item.name}}</i-option>
                </i-select>
                <i-button v-if="!fromData.brandNo" @click="addSeries" type="info">新增</i-button>
              </div>
            </i-form-item>
          </i-col>
        </i-row>
        <i-row>
          <i-col span="12">
            <i-form-item label="车系名称" prop="seriesName" :rules="{required: true, message: '车系名称不能为空'}">
              <div class="flex-items">
                <i-select :disabled="!fromData.brandNo" v-model="fromData.seriesName" placeholder="">
          ...
          <i-col span="12">
            <i-form-item label="车型名称" :rules="{required: true, message: '车型名称不能为空'}" prop="modelName">
              <i-input :disabled="!fromData.brandNo" v-model="fromData.modelName" placeholder=""></i-input>
            </i-form-item>
          </i-col>
        </i-row>
        <i-row>
        ...
        <i-row>
            ...
            <i-form-item label="车型图片" prop="logo">
              ...
              <i-upload
                ...
                <div class="upload-image">
                  <div v-if="!fromData.modelImg">
                    <i-icon type="ios-cloud-upload" size="52" style="color: #3399ff"></i-icon>
                    <p>单击或拖动文件上传</p>
                    <i-spin fix v-if="fromData.modelFileUploading">
                      <Icon type="load-c" size=18 class="demo-spin-icon-load"></Icon>
                      <div style="margin-top: 10px">正在上传中，请勿关闭...</div>
                    </i-spin>
                  </div>
                  <img v-else height="95" :src="fromData.modelImg" alt="">
                </div>
              </i-upload>
            </i-form-item>
          </i-col>
          <i-col span="12">
            <i-form-item
              label="备注"
              prop="remark">
              <i-input type="textarea" :rows="3" v-model="fromData.remark" placeholder=""></i-input>
            </i-form-item>
          </i-col>
        </i-row>
